...
def plot_lines(df, x=None, y=None, title="", yaxis_title="", show=True, fig=None, log=False, c=None, name=None,
               secondary_y=False, heat=None, save_path=None, width=800, height=600, heat_size=10):
    if fig is None:
        fig = make_subplots(specs=[[{"secondary_y": True}]])
    ...
    for i, col_name in enumerate(col_names):
        fig.add_trace(
            go.Scatter(
                ...
                marker=None if heat is None else {'color': df[heat], 'colorscale': 'Inferno', 'size': heat_size},
